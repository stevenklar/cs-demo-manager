...
function buildRoundsQuery({ name, ...filters }: TeamFilters) {
  ...
  let query = db
    ...
    .select([
      'demos.map_name as mapName',
      sql<number>`SUM(CASE WHEN rounds.winner_name = ${sql`${name}`} THEN 1 ELSE 0 END)`.as('roundWinCount'),
      sql<number>`SUM(CASE WHEN rounds.winner_name IS NOT NULL AND rounds.winner_name != ${sql`${name}`} THEN 1 ELSE 0 END)`.as(
        'roundLostCount',
      ),
      sql<number>`SUM(CASE WHEN rounds.winner_name = ${sql`${name}`} AND rounds.winner_side = 3 THEN 1 ELSE 0 END)`.as(
        'roundWinCountAsCt',
      ),
      sql<number>`SUM(CASE WHEN rounds.winner_name = ${sql`${name}`} AND rounds.winner_side = 2 THEN 1 ELSE 0 END)`.as(
        'roundWinCountAsT',
      ),
