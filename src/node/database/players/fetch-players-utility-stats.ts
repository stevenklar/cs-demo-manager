...
async function fetchPlayersAverageEnemiesFlashed(steamIds: string[], filters?: MatchFilters) {
  const query = db
    ...
    .select(() => [
      sql<number>`ROUND(CAST(flashed_count AS REAL) / NULLIF(total_count, 0), 1)`.as('average_enemies_flashed'),
    ])
...
async function fetchPlayersAverageHeGrenadeDamage(steamIds: string[], filters?: MatchFilters) {
  const query = db
    ...
    .select(
      sql<number>`ROUND(CAST(total_health_damage AS REAL) / NULLIF(total_count, 0), 1)`.as('average_damage'),
    )
